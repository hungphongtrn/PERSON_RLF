    ...
    def torch_mask_tokens(self, inputs: Any, special_tokens_mask: Optional[Any] = None):
        ...
        if special_tokens_mask is None:
            special_tokens_mask = [
                ...
                for val in labels["input_ids"].tolist()
            ]
        ...
        labels["input_ids"][~masked_indices] = (
            self.tokenizer.pad_token_id
        )  # We only compute loss on masked tokens

        # 80% of the time, we replace masked input tokens with tokenizer.mask_token ([MASK])
        indices_replaced = (
            torch.bernoulli(torch.full(labels["input_ids"].shape, 0.8)).bool()
            & masked_indices
        )
        ...
        indices_random = (
            torch.bernoulli(torch.full(labels["input_ids"].shape, 0.5)).bool()
            & masked_indices
            & ~indices_replaced
        )
        random_words = torch.randint(
            len(self.tokenizer), labels["input_ids"].shape, dtype=torch.long
        )
