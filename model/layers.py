...
class AttentionPool2d(nn.Module):
    def __init__(
        self, spacial_dim, embed_dim: int, num_heads: int, output_dim: int = None
    ):
        super().__init__()
...
class CLIP(nn.Module):
    def __init__(
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            self.visual = VisionTransformer(
                input_resolution=image_resolution,  # type: ignore
    ...
    def load_param(self, state_dict):
        ...
        for k, v in param_dict.items():
            if (
                k == "visual.positional_embedding"
                and v.shape != self.visual.positional_embedding.shape
            ):
                v = resize_pos_embed(
                    v,
                ...
                and v.shape != self.positional_embedding.shape
            ):
            ...
            try:
                ...
                print(
                    f"===========================ERROR occur in copy {k}, {v.shape}========================="
                )
                print(
...
def convert_weights(model: nn.Module):
    ...
    def _convert_weights_to_fp16(l):  # type: ignore
